...
type DistribTree<'T> = 
	| Leaf
	| Branch of 'T * ICloudRef<DistribTree<'T>> * ICloudRef<DistribTree<'T>>
	
// initialize a simple distributed tree
[<Cloud>]
let rec populate (depth : int) : Cloud<DistribTree<int>> =
    cloud {
        if depth = 0 then
            return! CloudRef.New Leaf
        else
            let! left, right = populate (depth - 1) <||> populate (depth - 1)
            return! CloudRef.New <| Branch(depth, left, right)
    }
    
let treeRef = runtime.Run <@ populate 5 @>
\end{lstlisting}
Once initiated, the distributed structure can be traversed by future computations.
\begin{lstlisting}
[<Cloud>]
let rec getBranchCount (tRef : ICloudRef<DistribTree<int>>) =
    cloud {
        match tRef.Value with
        | Leaf -> return 0
        | Branch(_,l,r) ->
            let! lc, rc = getBranchCount l <||> getBranchCount r
            return 1 + lc + rc
    }
    
runtime.Run <@ getBranchCount treeRef @>
\end{lstlisting}

%Cloud refs can only be initialized and dereferenced, which means that they
