...
module internal CloudComputationUtils =
    ...
    let report (warnings : CloudWarning list) =
        match Shell.Settings with
        | Some conf when conf.Verbose && not warnings.IsEmpty ->
            warnings |> Seq.iter (printf "%O")
        | _ -> ()
...
type internal CloudComputation<'T> (expr : Expr<ICloud<'T>>, dependencies : AssemblyContainer [], 
                                        warnings : CloudWarning list, ?name : string) =
