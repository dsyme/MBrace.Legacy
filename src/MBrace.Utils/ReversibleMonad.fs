module Nessos.MBrace.Utils.Reversible

    open System
    open System.Threading

    open Nessos.MBrace.Utils
    open Nessos.MBrace.Utils.LazyList
...
//        type Reversal with
//            static member ExecuteAsync (workflow : Reversal, ?runSequentially) : Async<unit> =
//                let runSequentially = defaultArg runSequentially false
//                evalRecovery runSequentially id workflow
//
//            static member Execute (workflow, ?runSequentially, ?timeout, ?cancellationToken) : unit = 
//                let wf = Reversal.ExecuteAsync(workflow, ?runSequentially = runSequentially)
//                Async.RunSynchronously(wf, ?timeout = timeout, ?cancellationToken = cancellationToken)
//            
