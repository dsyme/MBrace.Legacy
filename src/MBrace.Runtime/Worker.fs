...
let workerBehavior (processId: ProcessId)
    ...
    let toCloudRef cloudExpr = config.CloudRefProvider.CreateNewUntyped("temp" + (string processId), Guid.NewGuid().ToString(), cloudExpr, cloudExpr.GetType())
        ...
        let rec processTask' stack = 
            async {
                ...
                | ValueExpr (Obj (ObjValue value, t)) :: rest when value <> null ->
                    let! cloudRefValue = config.CloudRefProvider.CreateNew<obj>("temp" + (string processId), Guid.NewGuid().ToString(), box value)
