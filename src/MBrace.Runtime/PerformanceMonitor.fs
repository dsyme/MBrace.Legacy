...
module internal PerformanceMonitor =
        ...
        let private totalCpu =
            ...
            then let pc = new PerformanceCounter("Processor", "% Processor Time", "_Total",true)
        ...
        let private totalMemory = 
            if onMono then
                ...
                then let pc = new PerformanceCounter("Mono Memory", "Total Physical Memory")
                     Some <| PerformanceCounter (fun () -> Single (pc.NextValue()))
                else None
            else
        ...
        let private totalNetworkUsage =
            ...
            then let inst = (new PerformanceCounterCategory("Network Interface")).GetInstanceNames()
                 let pc = inst |> Array.map (fun nic ->
...
//        let private perfCounterAgent = new MailboxProcessor<AsyncReplyChannel<NodePerformanceInfo>>(fun inbox ->
